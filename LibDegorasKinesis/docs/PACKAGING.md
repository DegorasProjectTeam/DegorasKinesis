...
LibDegorasKinesis installs a single prefix-relative layout (via `GNUInstallDirs`) that serves **both** a standard
...
| Path | Contents |
|------|----------|
| `bin/` | `LibDegorasKinesis.dll` and (opt-out) the vendored Thorlabs runtime DLLs |
| `lib/` | import/static library |
| `include/LibDegorasKinesis/` | public headers |
...
|--------|---------|--------|
| `LIBDEGORASKINESIS_INSTALL` | ON if top-level | Master switch for all install/export rules |
| `LIBDEGORASKINESIS_BUILD_SHARED` | follows `BUILD_SHARED_LIBS`, else ON | Shared vs static |
| `LIBDEGORASKINESIS_INSTALL_THORLABS_RUNTIME` | ON | Install the vendored Thorlabs `*.dll` into `bin/` |
| `LIBDEGORASKINESIS_BUILD_DOCS` | OFF | Install `README.md` into the doc dir |
| `LIBDEGORASKINESIS_INSTALL_EXAMPLES` | OFF | Install the example sources |
| `LIBDEGORASKINESIS_BUILD_TOOLS` | OFF | Build/install CLI tools (none yet) |
| `LIBDEGORASKINESIS_BUILD_TESTING` | ON | Build tests and register them with CTest |
...
- `ports/libdegoras-kinesis/{vcpkg.json,portfile.cmake}` — the port.

### Manifest or classic mode

```sh
# classic mode
vcpkg install libdegoras-kinesis \
    --overlay-ports=LibDegorasKinesis/ports \
    --overlay-triplets=LibDegorasKinesis/cmake/triplets \
    --triplet x64-mingw-dynamic
```

```json
// manifest mode (vcpkg.json in your project), then `vcpkg install`
{ "dependencies": ["libdegoras-kinesis"] }
```
