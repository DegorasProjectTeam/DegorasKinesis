...
using dpkin::kinesis::MotorStatusFlags;
using dpkin::kinesis::decodeMotorStatus;
...
MotorStatusFlags decode(unsigned long raw)
{
    return decodeMotorStatus(std::bitset<32>(raw));
}

void testStatusDecodeSingleBits()
{
    // All-zero word: nothing set.
    MotorStatusFlags z = decode(0);
...
void testStatusDecodeDigitalInputs()
{
    ...
    for (int i = 0; i < 6; ++i)
    {
        MotorStatusFlags f = decode(din[i]);
        for (int j = 0; j < 6; ++j)
...
{
    MotorStatusFlags f = decode(0x00000400 | 0x80000000 | 0x00000010 | 0x00400000);
...
{
    std::mutex& a1 = dpkin::kinesis::serialMtx("SN-A");
    std::mutex& a2 = dpkin::kinesis::serialMtx("SN-A");
    std::mutex& b  = dpkin::kinesis::serialMtx("SN-B");
    assert(&a1 == &a2);   // same serial -> same mutex
    assert(&a1 != &b);    // distinct serials -> distinct mutexes

    std::mutex& d1 = dpkin::kinesis::discoveryMtx();
    std::mutex& d2 = dpkin::kinesis::discoveryMtx();
    assert(&d1 == &d2);
...
int main()
{
    ...
    std::cout << "UT_DCServoPure: ALL CHECKS PASSED" << std::endl;
    return 0;
}
