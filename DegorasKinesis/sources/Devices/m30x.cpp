...
// PROJECT INCLUDES
#include "DegorasKinesis/Devices/m30x.h"
...
OperationResult M30X::checkChannel(Channel ch)
{
...
OperationResult M30X::doMoveAbsolute(Channel ch, double real_pos)
{
    const OperationResult chk = this->checkChannel(ch);
    if (chk != OperationResult::OPERATION_OK)
        return chk;
...
OperationResult M30X::doMoveRelative(Channel ch, double real_pos)
{
    const OperationResult chk = this->checkChannel(ch);
    if (chk != OperationResult::OPERATION_OK)
        return chk;
...
OperationResult M30X::getChannelPosition(Channel ch, double& real_pos)
{
    real_pos = 0.0;
    const OperationResult chk = this->checkChannel(ch);
    if (chk != OperationResult::OPERATION_OK)
...
OperationResult M30X::getChannelFlags(Channel ch, kinesis::MotorStatusFlags& flags)
{
