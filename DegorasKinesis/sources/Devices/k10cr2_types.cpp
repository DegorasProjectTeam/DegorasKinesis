...
// PROJECT INCLUDES
#include "DegorasKinesis/Devices/k10cr2_types.h"
...
K10CR2ChannelStatus::K10CR2ChannelStatus() :
    valid(false),
    flags(),
    pos_raw(0),
    real_pos(0.0)
...
std::string K10CR2ChannelStatus::toJsonStr(bool pretty) const
{
...
K10CR2ChannelStatus K10CR2ChannelStatus::fromJsonStr(const std::string& json_str)
{
    K10CR2ChannelStatus status;
...
K10CR2DeviceStatus::K10CR2DeviceStatus() :
    serial_no(),
    connected(false),
    chann()
{}

std::string K10CR2DeviceStatus::toJsonStr(bool pretty) const
{
    std::ostringstream ss;
    ss << "{"
...
K10CR2DeviceStatus K10CR2DeviceStatus::fromJsonStr(const std::string& json_str)
{
    K10CR2DeviceStatus status;
    status.serial_no = json::getString(json_str, "serial_no");
    status.connected = json::getBool(json_str, "connected");
    status.chann = K10CR2ChannelStatus::fromJsonStr(json::getObject(json_str, "chann"));
    return status;
}
