...
/**
 * @brief Decoded Benchtop DC Servo status word.
 * @note The bit layout is specific to the Kinesis Benchtop.DCServo module. A device on a different Kinesis module
 *       has a different status layout and must provide its own decoder; this struct must never be reused for one.
 */
struct LIBTHORLABSKINESIS_EXPORT DCServoStatusFlags
{
...
/**
 * @brief Decode a raw Benchtop DC Servo 32-bit status word into named flags.
 * @param bits The status word as returned by BDC_GetStatusBits.
 * @return The decoded flags.
 * @note Pure function with no vendor-SDK dependency, so it is unit-testable without hardware.
 */
LIBTHORLABSKINESIS_EXPORT DCServoStatusFlags decodeDCServoStatus(const std::bitset<32>& bits);
