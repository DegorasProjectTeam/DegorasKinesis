...
/**
 * @brief Map a raw Kinesis FT_ / BDC_ return code to a high-level OperationResult category.
 * @param code The raw short returned by a Kinesis call.
 * @return The category. The raw code is preserved separately in DeviceError::kinesis_code.
 * @note Pure function (no vendor-SDK dependency in its interface), so it is unit-testable without hardware.
 */
LIBTHORLABSKINESIS_EXPORT types::OperationResult categoryFromKinesis(short code);
 ...
 */
class LIBTHORLABSKINESIS_EXPORT DCServoChannel
{
public:
