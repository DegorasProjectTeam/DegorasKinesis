...
#include <LibThorlabsKinesis/Modules/Common>
...
bool flagsEq(const dcservo::DCServoStatusFlags& a, const dcservo::DCServoStatusFlags& b)
{
...
void testFlags()
{
    ...
    assert(flagsEq(f, dcservo::DCServoStatusFlags::fromJsonStr(pretty)));   // pretty parses identically
}
...
void testM30XY()
{
    ...
    c.valid = true; c.pos_raw = 12345; c.pos_mm = 8.5;
    ...
    assert(crt.channel == c.channel && crt.valid == c.valid && crt.pos_raw == c.pos_raw
           && dEq(crt.pos_mm, c.pos_mm) && flagsEq(crt.flags, c.flags));

    M30XYDeviceStatus d;
    d.serial_no = "101000002"; d.connected = true;
    d.chann_x = M30XYChannelStatus(Channel::X_CHANNEL); d.chann_x.valid = true; d.chann_x.pos_mm = 1.5;
    d.chann_x.flags.homed = true;
    d.chann_y = M30XYChannelStatus(Channel::Y_CHANNEL); d.chann_y.valid = true; d.chann_y.pos_mm = 2.5;
    ...
    assert(drt.chann_x.channel == Channel::X_CHANNEL && dEq(drt.chann_x.pos_mm, 1.5) && drt.chann_x.flags.homed);
    assert(drt.chann_y.channel == Channel::Y_CHANNEL && dEq(drt.chann_y.pos_mm, 2.5) && drt.chann_y.flags.enabled);
}
...
void testM30X()
{
    ...
    d.chann.valid = true; d.chann.pos_mm = 3.5; d.chann.flags.homed = true; d.chann.flags.limit_cw = true;
    ...
    assert(rt.serial_no == d.serial_no && rt.connected && rt.chann.valid
           && dEq(rt.chann.pos_mm, 3.5) && rt.chann.flags.homed && rt.chann.flags.limit_cw);
